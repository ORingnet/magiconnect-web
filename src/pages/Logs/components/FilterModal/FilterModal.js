...
import { actionOptions, levelOptions } from './options';
import { Row, Col, Input } from 'reactstrap';
import { StyledDatePickerBox } from './StyledFilterModal';
const today = new Date();
today.setDate(today.getDate());
const FilterModal = ({ modalIsOpen, toggleModal, filterLogs }) => {
  ...
  const renderLevelOptions = levelOptions.map(level => (
    <FormattedMessage id={level.formattedId} key={level.value}>
      {message => <option value={level.value}>{message}</option>}
    </FormattedMessage>
  ));
  return (
    <Modal
